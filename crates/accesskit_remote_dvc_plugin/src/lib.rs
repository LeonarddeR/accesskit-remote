...
unsafe fn get_instance(refiid: *const GUID, pnumobjs: *mut u32, ppo: *mut *mut c_void) -> HRESULT {
    ...
    if ppo.is_null() {
        // Probe: report how many plug-ins we implement.
        *pnumobjs = 1;
        debug!("VirtualChannelGetInstance probe: reporting {} plugin(s)", *pnumobjs);
        return S_OK;
    }

    // Fetch: never write past the array the caller allocated.
    let cap = *pnumobjs as usize;
    debug!("VirtualChannelGetInstance fetch: caller array cap = {cap}");
    if cap < 1 {
    ...
    unsafe { *ppo = plugin.into_raw() };
    *pnumobjs = 1;
    debug!("VirtualChannelGetInstance fetch: wrote 1 plugin");
    S_OK
}
